...
@Component
public class BidCalculatorInMemory
        implements BidCalculatorInterface
{
    ...
    public final static Logger                   logger              = LoggerFactory
            .getLogger(BidCalculatorInMemory.class);

    public Comparator<BidModel>                   bidComparator       = new Comparator<BidModel>()
                                                                     {

                                                                         @Override
                                                                         public int compare(BidModel m1, BidModel m2)
                                                                         {
                                                                             return m2.getBidValue()
                                                                                     .compareTo(m1.getBidValue());
                                                                         }
                                                                     };
    ...
    @Override
    public void addBid(Long projectId, BidModel current)
            throws BidExpiredException
    {
	
	Timestamp expiryTime = projectTimestampMap.get(projectId);

        if ( expiryTime == null || System.currentTimeMillis() > expiryTime.getTime() ) throw new BidExpiredException();
        SortedSet<BidModel> top3 = new TreeSet<BidModel>(bidComparator);
        top3 = projectBidMap.get(projectId);

        if ( top3 == null ) throw new BidExpiredException();
        

        BigDecimal smallestOfTheTop3 = top3.last() == null ? null : top3.last().getBidValue();
        if ( smallestOfTheTop3 == null || current.getBidValue().compareTo(smallestOfTheTop3) > 0 )
        {
            top3.add(current);
            if ( top3.size() > 3 ) top3.remove(top3.last());
        }
    ...
    public String toString()
    {
        ...
        while (iter.hasNext())
        {
            ...
            for (Object bdObj : arr)
            {
                ...
                ret = ret + bd.getBidId() + "->" + bd.getBidValue() + ",";
            }
            ret = ret + "]   ;   ";
        }
        return ret;
